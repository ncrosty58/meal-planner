...
from recipe_nutrition import impute_recipe_nutrition, parse_nutrient_val
...
def find_recipe_for_ingredient(ingredient, all_recipes=None):
    ...
    for r in all_recipes:
        name = r['name']
        # Remove (1), (2) etc suffixes for better matching
...
def find_and_import_recipe(ingredient, existing_recipe_ids=[]) -> bool:
    ...
    for url in potential_links:
        ...
        if url in existing_urls:
            ...
            return True # Treat as success so the caller can find it in the DB
        
        # Validation prompt (Delegated to Mealie Recipe Link Validator Skill)
        validation_prompt = (
        ...
        try:
            ...
            if "YES" in val_res:
                ...
                if r.status_code in (200, 201):
                    ...
                    try:
                        recipe_details = client.get_recipe_details(slug)
                        # Mealie usually has an image ID if an image exists
                        if not recipe_details.get('image'):
                            print(f"[Recipe Finder] Imported recipe '{slug}' has no image. Deleting and skipping...")
                            client.delete_recipe(recipe_details['id'])
                            continue
                        ...
                        if not db_nutrition or any(db_nutrition.get(f) is None or db_nutrition.get(f) == "" or db_nutrition.get(f) == 0 or db_nutrition.get(f) == "0" for f in key_fields):
                            recipe_details = impute_recipe_nutrition(recipe_details, client)
