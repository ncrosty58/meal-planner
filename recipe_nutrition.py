...
from config import (
    RDA, BREAKFAST_PROFILES, LUNCH_LEFTOVER_PROFILE, LUNCH_SANDWICH_PROFILE, load_skill_md
)
...
def impute_recipe_nutrition(recipe, client=None):
    ...
    try:
        ...
        updated_recipe['nutrition'] = {
            ...
            "transFatContent": nutrition_data.get('transFatContent'),
            "unsaturatedFatContent": nutrition_data.get('unsaturatedFatContent')
        }
        
        # Save back to Mealie
        r_put = requests.put(f"{client.api_url}/api/recipes/{slug}", json=updated_recipe, headers=client.headers)
            ...
            if is_active:
                if recipe_id:
                    try:
                        ...
                        if is_incomplete:
                            # Automatically impute and reload details
                            recipe = impute_recipe_nutrition(recipe, client)
                            db_nutrition = recipe.get('nutrition', {})
